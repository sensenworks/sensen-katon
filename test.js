...
const main = () => {
    ...
    return Builder(Widget(mystate.use(), Style({
        ...
        color: '#F90000',
        fontSize: '48px'
    })), Textual('{{ firstname }} ', UseKit({
        color: '#777',
        fontSize: '24px'
    })), Textual('{{ lastname }}, {{ hello }} ', UseKit({
        color: '#777',
        fontSize: '24px'
    })), mystate.watch('checker')
        .widget(({ checker }) => checker == true
        ? Textual('FirstName is Valid', Style({ backgroundColor: 'green', color: 'white', padding: '.4rem' }))
        : Textual('FirstName is not valid', Style({ backgroundColor: 'red', color: 'white', padding: '.4rem' }))), paragraph(), Textual('License MIT'), Form(Attribution({
        action: '#',
        method: 'post'
    }), Column(Row(Input({
        type: 'text',
...
const triggerAction = Action('message', ({ builder, event }) => {
